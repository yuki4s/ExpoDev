...
def start_log_recording():
    ...
    color_path = os.path.join(video_log_dir, f"{branch_name}_log{log_number}_ID{ID}_Cond{Cond}_color.mp4")
    depth_path = os.path.join(video_log_dir, f"{branch_name}_log{log_number}_ID{ID}_Cond{Cond}_depth.mp4")
    ...
    frame_index = 0
    recording = True
    print(f"[VM] ログ記録開始: {branch_name}_log{log_number}_ID{ID}_Cond{Cond}")
...
def stop_log_recording():
    ...
    branch_name = get_git_branch_name().replace("/", "_")  # ← ブランチ名取得
    json_path = os.path.join(landmark_log_dir, f"{branch_name}_log{log_number}_ID{ID}_Cond{Cond}_handLandmarks.json")
